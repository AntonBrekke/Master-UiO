...
class Pandemolator(object):
    def __init__(self, m_chi, k_chi, dof_chi, m_X, k_X, dof_X, m_h, k_h, dof_h, m_psi, k_psi, C_n, C_rho, C_xi0, t_grid, T_grid, dT_dt_grid, ent_grid, hubble_grid, sf_grid, i_ic, n_ic, rho_ic, i_end):
    ...
    def n_h(self, T_chi, xi_h):
        return dens.n(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)

    def rho(self, T_chi, xi_chi, xi_X, xi_h):
        return dens.rho(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.rho(self.k_X, T_chi, self.m_X, self.dof_X, xi_X) + dens.rho(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)

    def P(self, T_chi, xi_chi, xi_X, xi_h):
        return dens.P(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.P(self.k_X, T_chi, self.m_X, self.dof_X, xi_X) + dens.P(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)

    def rho_3P_diff(self, T_chi, xi_chi, xi_X, xi_h):
        return dens.rho_3P_diff(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.rho_3P_diff(self.k_X, T_chi, self.m_X, self.dof_X, xi_X) + dens.rho_3P_diff(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)
    ...
    def n_h_der_T(self, T_chi, xi_h):
        return dens.n_der_T(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)
    
    def n_h_der_xi(self, T_chi, xi_h):
        return dens.n_der_xi(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)

    def rho_der_T(self, T_chi, xi_chi, xi_X, xi_h):
        return dens.rho_der_T(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.rho_der_T(self.k_X, T_chi, self.m_X, self.dof_X, xi_X) + dens.rho_der_T(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)
    ...
    def rho_h_der_xi(self, T_chi, xi_h):
        return dens.rho_der_xi(self.k_h, T_chi, self.m_h, self.dof_h, xi_h)

    def n_rho_root(self, Txi_chi, n_in, rho_in):
        T_chi = exp(max(min(Txi_chi[0], 10.), -100.))
        xi_chi = min(Txi_chi[1]+self.m_chi/T_chi, (1.-1e-14)*self.m_X/(self.fac_n_X*T_chi), (1.-1e-14)*self.m_h/(self.fac_n_h*T_chi))
        n = max(self.n_chi(T_chi, xi_chi) + self.fac_n_X*self.n_X(T_chi, self.fac_n_X*xi_chi) + self.fac_n_h*self.n_h(T_chi, self.fac_n_h*xi_chi), 1e-300)
        rho = max(self.rho(T_chi, xi_chi, self.fac_n_X*xi_chi, self.fac_n_h*xi_chi), 1e-300)
    ...
    def jac_n_rho_root(self, Txi_chi, n_in, rho_in):
        ...
        n = max(self.n_chi(T_chi, xi_chi) + self.fac_n_X*self.n_X(T_chi, self.fac_n_X*xi_chi) + self.fac_n_h*self.n_h(T_chi, self.fac_n_h*xi_chi), 1e-300)
        rho = max(self.rho(T_chi, xi_chi, self.fac_n_X*xi_chi, self.fac_n_h*xi_chi), 1e-300)
        n_der_T = self.n_chi_der_T(T_chi, xi_chi) + self.fac_n_X*self.n_X_der_T(T_chi, self.fac_n_X*xi_chi) + self.fac_n_h*self.n_h_der_T(T_chi, self.fac_n_h*xi_chi)
        n_der_xi = self.n_chi_der_xi(T_chi, xi_chi) + self.fac_n_X*self.fac_n_X*self.n_X_der_xi(T_chi, self.fac_n_X*xi_chi) + self.fac_n_h*self.fac_n_h*self.n_h_der_xi(T_chi, self.fac_n_h*xi_chi)
        rho_der_T = self.rho_der_T(T_chi, xi_chi, self.fac_n_X*xi_chi, self.fac_n_h*xi_chi)
        rho_der_xi = self.rho_chi_der_xi(T_chi, xi_chi) + self.fac_n_X*self.rho_X_der_xi(T_chi, self.fac_n_X*xi_chi) + self.fac_n_h*self.rho_h_der_xi(T_chi, self.fac_n_h*xi_chi)
    ...
    def rho_root(self, log_T_chi, rho_in):
        ...
        rho = max(self.rho(T_chi, 0., 0., 0.), 1e-300)
        return [log(rho/rho_in)]

    def jac_rho_root(self, log_T_chi, rho_in):
        T_chi = exp(max(min(log_T_chi[0], 10.), -100.))
        rho = max(self.rho(T_chi, 0., 0., 0.), 1e-300)
        rho_der_T = self.rho_der_T(T_chi, 0., 0., 0.)
    ...
    def der(self, log_x, y):
        ...
        xi_chi = min(root_sol.x[1] + self.m_chi/T_chi, (1.-1e-14)*self.m_X/(self.fac_n_X*T_chi), (1.-1e-14)*self.m_h/(self.fac_n_h*T_chi))
        xi_X = self.fac_n_X*xi_chi
        xi_h = self.fac_n_h*xi_chi
        ...
        n_sol = self.n_chi(T_chi, xi_chi) + self.fac_n_X*self.n_X(T_chi, xi_X) + self.fac_n_h*self.n_h(T_chi, xi_h)
        rho_sol = self.rho(T_chi, xi_chi, xi_X, xi_h)
        ...
        C_n = self.C_n(T, T_chi, xi_chi, xi_X, xi_h)
        C_rho = self.C_rho(T, T_chi, xi_chi, xi_X, xi_h)
        # print('Rel. density changes:', C_n/(3.*H*n), C_rho/(3.*H*(rho+P)))

        der_Y = -(T/dT_dt)*C_n/ent
        der_rho = -(T/dT_dt)*(H*self.rho_3P_diff(T_chi, xi_chi, xi_X, xi_h) + C_rho)*(sf**4.)
    ...
    def event_xi_zero(self, log_x, y):
        ...
        C_xi0 = self.C_xi0(T, T_chi, xi_chi, xi_X, xi_h)
        # print(C_xi0/(H*n))
    ...
    def der_xi_0(self, log_x, y):
        ...
        C_rho = self.C_rho(T, T_chi, 0., 0., 0.)
        # print('Rel. density changes:', C_rho/(3.*H*(rho+P)))

        der_rho = -(T/dT_dt)*(H*self.rho_3P_diff(T_chi, 0., 0., 0.) + C_rho)*(sf**4.)
    ...
    def event_xi_nonzero(self, log_x, y):
        ...
        C_xi0 = self.C_xi0(T, T_chi, 0., 0., 0.)
        n_chi = self.n_chi(T_chi, 0.)
        n_X = self.n_X(T_chi, 0.)
        n_h = self.n_h(T_chi, 0.)
    ...
    def event_abund_large_xi_0(self, log_x, y):
        ...
        T_chi = exp(root_sol.x[0])
        self.T_chi_last, self.xi_chi_last = T_chi, 0.
        Y = (self.n_chi(T_chi, 0.) + self.fac_n_X*self.n_X(T_chi, 0.) + self.fac_n_h*self.n_h(T_chi, 0.)) / ent
        Odh2_today = Y * cf.s0 * self.m_chi / cf.rho_crit0_h2
        return 1. - Odh2_today/(fac_abund_stop*cf.omega_d0)
    ...
    def pandemolate(self):
        ...
        dof_fac_h = self.dof_h if self.k_h == -1 else self.dof_h*7./8.
        self.T_chi_last = (self.rho_ic / (cf.pi2*(dof_fac_chi+dof_fac_X+dof_fac_h)/30.))**0.25
        self.xi_chi_last = 0.
        ...
        while i_max < n_pts - 1:
            ...
            if i_max > 0:#self.event_xi_nonzero(self.log_x_pts[i_max], [rho0*(sf0**4.)]) > 0.: # xi = 0 at beginning of calculation
                ...
                self.T_chi_last = (rho0 / (cf.pi2*(dof_fac_chi+dof_fac_X+dof_fac_h)/30.))**0.25
                self.xi_chi_last = 0.
                for i in range(i_max, i_xi_nonzero + 1):
                    ...
                    self.xi_h_grid_sol[i] = self.fac_n_h*self.xi_chi_grid_sol[i]
                    ...
                    self.n_h_grid_sol[i] = self.n_h(self.T_chi_grid_sol[i], self.xi_h_grid_sol[i])
                ...
                if sol_xi0.t_events[1].size == 0: # abundance always < fac_abund_stop*DM abundance
                    ...
                    n0 = self.n_chi(self.T_chi_last, 0.) + self.fac_n_X*self.n_X(self.T_chi_last, 0.) + self.fac_n_h*self.n_h(self.T_chi_last, 0.)
            ...
            if i_xi_nonzero < n_pts - 1:
                ...
                for i in range(i_start, i_max + 1):
                    ...
                    self.T_chi_grid_sol[i] = exp(root_sol.x[0])
                    self.xi_chi_grid_sol[i] = min(root_sol.x[1] + self.m_chi/self.T_chi_grid_sol[i], (1.-1e-14)*self.m_X/(self.fac_n_X*self.T_chi_grid_sol[i]), (1.-1e-14)*self.m_h/(self.fac_n_h*self.T_chi_grid_sol[i]))#root_sol.x[1] + self.m_chi/self.T_chi_grid_sol[i]
                    self.T_chi_last, self.xi_chi_last = self.T_chi_grid_sol[i], self.xi_chi_grid_sol[i]
                    self.xi_X_grid_sol[i] = self.fac_n_X*self.xi_chi_grid_sol[i]
                    self.xi_h_grid_sol[i] = self.fac_n_h*self.xi_chi_grid_sol[i]
                    self.n_chi_grid_sol[i] = self.n_chi(self.T_chi_grid_sol[i], self.xi_chi_grid_sol[i])
