...
xih = data[::data_skip_pan, 9]
nd = data[::data_skip_pan, 10]
nX = data[::data_skip_pan, 11]
nh = data[::data_skip_pan, 12]
...
if BP != 5:
    ...
    ax1.text(md/T_nu[np.where(mX*nX/ent==YX_max)]*6, YX_max*1e-1, r'$X_\mu$', color=c3, ha='center', va='top')
    ax1.text(md/T_nu[np.where(mh*nh/ent==Yh_max)]*2, Yh_max*1e-1, r'$h_{\phi}$', color=c2, ha='left', va='bottom')
...
xi_h_grid = data_evo[::data_skip_rate,9]
n_d_grid = data_evo[::data_skip_rate,10]
n_X_grid = data_evo[::data_skip_rate,11]
n_h_grid = data_evo[::data_skip_rate,12]
...
ax3.loglog(x_grid, 1e6*abs(C_X_aa), color='brown', ls='-')
ax3.loglog(x_grid, 1e6*abs(C_aa_X), color='purple', ls='-')
...
ax3.loglog(x_grid, 1e6*abs(C_dd_h), color=c1, ls='--', zorder=-4) #114B5F
ax3.loglog(x_grid, 1e6*abs(C_da_h), color=c2, ls='--', zorder=-4) #458751
ax3.loglog(x_grid, 1e6*abs(C_h_aa), color='brown', ls='--')
ax3.loglog(x_grid, 1e6*abs(C_aa_h), color='purple', ls='--')
ax3.loglog(x_grid, 1e6*abs(C_dd_hh), color=c3, ls='--', zorder=-4) #95190C
ax3.loglog(x_grid, 1e6*abs(C_hh_dd), color=c5, ls='--', zorder=-4) #D02411
ax3.loglog(x_grid, 1e6*abs(C_h_XX), color=c5, ls='--', zorder=-4) #D02411
...
if BP == 1: 
    ...
    y_sa_P = 5e2
    y_PP_ss = 0
    ax3.text(0.3, y_sa_P*np.abs(1e6*C_h_XX[x_therm_index]), r"$h_\phi \to XX$", color=c5, rotation=0, ha='left', va='top')
else: 
    x_ss_PP = 30.3
    x_PP_ss = 30.3
    x_sa_P = 1.3
...
ax3.text(x_therm*x_sa_P, y_sa_P*np.abs(1e6*C_da_h[x_therm_index]), r"$\nu_s \nu_\alpha \to \Phi$", color=c2, rotation=0, ha='left', va='top')
ax3.text(x_therm*x_ss_PP, y_ss_PP*np.abs(1e6*C_dd_XX[x_therm_index]), r"$\nu_s \nu_s \to \Phi \Phi$", color=c3, rotation=0, ha='left', va='bottom')
ax3.text(x_therm*x_PP_ss, y_PP_ss, r"$\Phi \Phi \to \nu_s \nu_s$", color=c5, rotation=0, va='bottom')

ax3.plot([1e-10, 1e-9], [1e-40, 1e-35], linestyle='-', color='black', label=r'$\Phi=X_\mu$')
ax3.plot([1e-10, 1e-9], [1e-40, 1e-35], linestyle='--', color='black', label=r'$\Phi=h_\phi$')
BP_str = r'$\textit{BP' + f'{BP}' + r'}$'
