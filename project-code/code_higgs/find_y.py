...
r_m_Xd = 3.        # Anton: Mass ratio m_X/m_d
r_m_hd = 3.        # Anton: Mass ratio m_h/m_d
...
def find_y(params):
    ...
    if log_enhance_req < 0.:
        ...
        return m_d, m_X, m_h, sin2_2th, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan
    ...
    for i in range(i_max + 1):
        ...
        try:
            ...
            # t, T_SM, T_nu, ent, H, sf, T_d, xi_d, xi_X, xi_h, n_d, n_X, n_h, C_therm, fs_length, fs_length_3, T_kd, T_kd_3, T_d_kd, T_d_kd_3, r_sound, r_sound_3, reached_integration_end
            t_grid, T_SM_grid, T_nu_grid, ent_grid, hubble_grid, sf_grid, T_d_grid, xi_d_grid, xi_X_grid, xi_h_grid, n_d_grid, n_X_grid, n_h_grid, C_therm_grid, fs_length, fs_length_3, T_kd, T_kd_3, T_d_kd, T_d_kd_3, r_sound, r_sound_3, reached_integration_end = sterile_caller.call(m_d, m_X, m_h, m_a, k_d, k_X, k_h, k_a, dof_d, dof_X, dof_h, sin2_2th, y_cur, spin_facs=spin_facs, off_shell=off_shell)
    ...
    mh_str = f'{m_h:.5e}'.split('e')[0].rstrip('0').rstrip('.') + 'e' + f'{m_h:.5e}'.split('e')[1]
    ...
    filename = f'md_{md_str};mX_{mX_str};mh_{mh_str};sin22th_{sin22th_str};y_{y_str};full.dat'
    ...
    return m_d, m_X, m_h, sin2_2th, y_cur, O_d_h2, x_therm, x_d_therm, therm_ratio_max, fs_length, fs_length_3, T_kd, T_kd_3, T_d_kd, T_d_kd_3, r_sound, r_sound_3
...
if __name__ == '__main__':
    ...
    hours = 12
    minutes = 0
    timeout = days*24*60*60 + hours*60*60 + minutes*60      # Anton: Set to None for no timeout
    # timeout = None
    ...
    with ProcessPool(max_workers=num_process) as pool:
        ...
        filename_params = dirname+f'rmXd_{r_m_Xd:.2e}_rmhd_{r_m_hd:.2e}_y_relic_test_{n_m}x{n_th}x{i_max}.dat'
        ...
        while True:
            ...
            try:
                ...
                m_X = r_m_Xd*m_d 
                m_h = r_m_hd*m_d 
                result = (m_d, m_X, m_h, sin2_2th, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
                result_list.append(result)
                openfile.write(str(result).strip('()').replace(',', '')+'\n')
    ...
    print(dirname+f'rmXd_{r_m_Xd:.2e}_rmhd_{r_m_hd:.2e}_y_relic_test_{n_m}x{n_th}x{i_max}.dat')
    np.savetxt(dirname+f'rmXd_{r_m_Xd:.2e}_rmhd_{r_m_hd:.2e}_y_relic_test_{n_m}x{n_th}x{i_max}.dat', results)
