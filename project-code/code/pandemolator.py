...
class Pandemolator(object):
    def __init__(self, m_chi, k_chi, dof_chi, m_X, k_X, dof_X, m_psi, k_psi, C_n, C_rho, C_xi0, t_grid, T_grid, dT_dt_grid, ent_grid, hubble_grid, sf_grid, i_ic, n_ic, rho_ic, i_end):
    ...
    def rho(self, T_chi, xi_chi, xi_X):
        return dens.rho(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.rho(self.k_X, T_chi, self.m_X, self.dof_X, xi_X)

    def P(self, T_chi, xi_chi, xi_X):
        return dens.P(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.P(self.k_X, T_chi, self.m_X, self.dof_X, xi_X)

    def rho_3P_diff(self, T_chi, xi_chi, xi_X):
        return dens.rho_3P_diff(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.rho_3P_diff(self.k_X, T_chi, self.m_X, self.dof_X, xi_X)
    ...
    def rho_der_T(self, T_chi, xi_chi, xi_X):
        return dens.rho_der_T(self.k_chi, T_chi, self.m_chi, self.dof_chi, xi_chi) + dens.rho_der_T(self.k_X, T_chi, self.m_X, self.dof_X, xi_X)
    ...
    def n_rho_root(self, Txi_chi, n_in, rho_in):
        ...
        n = max(self.n_chi(T_chi, xi_chi) + self.fac_n_X*self.n_X(T_chi, self.fac_n_X*xi_chi), 1e-300)
    ...
    def jac_n_rho_root(self, Txi_chi, n_in, rho_in):
        ...
        n = max(self.n_chi(T_chi, xi_chi) + self.fac_n_X*self.n_X(T_chi, self.fac_n_X*xi_chi), 1e-300)
        rho = max(self.rho(T_chi, xi_chi, self.fac_n_X*xi_chi), 1e-300)
        n_der_T = self.n_chi_der_T(T_chi, xi_chi) + self.fac_n_X*self.n_X_der_T(T_chi, self.fac_n_X*xi_chi)
        n_der_xi = self.n_chi_der_xi(T_chi, xi_chi) + self.fac_n_X*self.fac_n_X*self.n_X_der_xi(T_chi, self.fac_n_X*xi_chi)
    ...
    def der(self, log_x, y):
        ...
        n_sol = self.n_chi(T_chi, xi_chi) + self.fac_n_X*self.n_X(T_chi, xi_X)
    ...
    def event_xi_nonzero(self, log_x, y):
        ...
        return C_xi0/(xi_ratio_stop*H*(n_chi + 2.*n_X)) - 1.
    ...
    def event_abund_large_xi_0(self, log_x, y):
        ...
        T_chi = exp(root_sol.x[0])
        self.T_chi_last, self.xi_chi_last = T_chi, 0.
        Y = (self.n_chi(T_chi, 0.) + self.fac_n_X*self.n_X(T_chi, 0.)) / ent
        Odh2_today = Y * cf.s0 * self.m_chi / cf.rho_crit0_h2
        return 1. - Odh2_today/(fac_abund_stop*cf.omega_d0)
        ...
        while i_max < n_pts - 1:
            ...
            if i_max > 0:#self.event_xi_nonzero(self.log_x_pts[i_max], [rho0*(sf0**4.)]) > 0.: # xi = 0 at beginning of calculation
                ...
                if sol_xi0.t_events[1].size == 0: # abundance always < fac_abund_stop*DM abundance
                    ...
                    n0 = self.n_chi(self.T_chi_last, 0.) + self.fac_n_X*self.n_X(self.T_chi_last, 0.)
