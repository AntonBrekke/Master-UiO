...
class Pandemolator(object):
    def __init__(self, m_chi, k_chi, dof_chi, m_phi, k_phi, dof_phi, m_psi, k_psi, C_n, C_rho, C_xi0, t_grid, T_grid, dT_dt_grid, ent_grid, hubble_grid, sf_grid, i_ic, n_ic, rho_ic, i_end):
        ...
        self.i_ic = i_ic
        self.n_ic = n_ic     # initial condition for n_chi + self.fac_n_phi*n_phi
    ...
    def rho_X_der_xi(self, T_chi, xi_phi):
    ...
    def jac_n_rho_root(self, Txi_chi, n_in, rho_in):
        ...
        n_der_xi = self.n_chi_der_xi(T_chi, xi_chi) + self.fac_n_phi*self.fac_n_phi*self.n_phi_der_xi(T_chi, self.fac_n_phi*xi_chi)
        rho_der_T = self.rho_der_T(T_chi, xi_chi, self.fac_n_phi*xi_chi)
        rho_der_xi = self.rho_chi_der_xi(T_chi, xi_chi) + self.fac_n_phi*self.rho_X_der_xi(T_chi, self.fac_n_phi*xi_chi)
    ...
    def der(self, log_x, y):
        ...
        # print(self.m_chi/T, T, self.m_chi/T_chi, Y * cf.s0 * self.m_chi / cf.rho_crit0_h2, rho, xi_chi, xi_phi)

        # P = self.P(T_chi, xi_chi, xi_phi)

        C_n = self.C_n(T, T_chi, xi_chi, xi_phi)
        C_rho = self.C_rho(T, T_chi, xi_chi, xi_phi)
        # print('Rel. density changes:', C_n/(3.*H*n), C_rho/(3.*H*(rho+P)))

        der_Y = -(T/dT_dt)*C_n/ent
        der_rho = -(T/dT_dt)*(H*self.rho_3P_diff(T_chi, xi_chi, xi_phi) + C_rho)*(sf**4.)
    ...
    def event_xi_nonzero(self, log_x, y):
        ...
        n_phi = self.n_phi(T_chi, 0.)
        # print(C_xi0, H*(n_chi + self.fac_n_phi*n_phi))

        return C_xi0/(xi_ratio_stop*H*(n_chi + 2.*n_phi)) - 1.
    ...
    def event_abund_large_xi_0(self, log_x, y):
        ...
        T_chi = exp(root_sol.x[0])
        self.T_chi_last, self.xi_chi_last = T_chi, 0.
        Y = (self.n_chi(T_chi, 0.) + self.fac_n_phi*self.n_phi(T_chi, 0.)) / ent
        Odh2_today = Y * cf.s0 * self.m_chi / cf.rho_crit0_h2
        return 1. - Odh2_today/(fac_abund_stop*cf.omega_d0)

    def pandemolate(self):
        dof_fac_chi = self.dof_chi if self.k_chi == -1 else self.dof_chi*7./8.
        dof_fac_X = self.dof_phi if self.k_phi == -1 else self.dof_phi*7./8.
        self.T_chi_last = (self.rho_ic / (cf.pi2*(dof_fac_chi+dof_fac_X)/30.))**0.25
        self.xi_chi_last = 0.
        ...
        self.xi_phi_grid_sol = np.empty(n_pts)
        self.n_chi_grid_sol = np.empty(n_pts)
        self.n_phi_grid_sol = np.empty(n_pts)
        ...
        while i_max < n_pts - 1:
            ...
            if i_max > 0:#self.event_xi_nonzero(self.log_x_pts[i_max], [rho0*(sf0**4.)]) > 0.: # xi = 0 at beginning of calculation
                ...
                for i in range(i_max, i_xi_nonzero + 1):
                    ...
                    self.n_phi_grid_sol[i] = self.n_phi(self.T_chi_grid_sol[i], self.xi_phi_grid_sol[i])
                ...
                if sol_xi0.t_events[1].size == 0: # abundance always < fac_abund_stop*DM abundance
                    ...
                    n0 = self.n_chi(self.T_chi_last, 0.) + self.fac_n_phi*self.n_phi(self.T_chi_last, 0.)
        ...
        self.xi_phi_grid_sol = self.xi_phi_grid_sol[:n_pts]
        self.n_chi_grid_sol = self.n_chi_grid_sol[:n_pts]
        self.n_phi_grid_sol = self.n_phi_grid_sol[:n_pts]
...
if __name__ == '__main__':
    ...
    # M2_X23 = 2*g**2/m_phi^2 * (m_phi2 - (m2 - m3)**2)*(2*m_phi2 + (m2 + m3)**2)
    # New matrix elements for X --> 23
    M2_dd = 2.*y2*(c_th**4.)/m_phi2 * (m_phi2)*(2*m_phi2 + (2*m_d)**2)
    M2_aa = 2.*y2*(s_th**4.)/m_phi2 * (m_phi2)*(2*m_phi2)
    M2_da = 2.*y2*(s_th**2.)*(c_th**2.)/m_phi2 * (m_phi2 - m_d**2)*(2*m_phi2 + m_d**2)

    def C_n(T_a, T_d, xi_d, xi_phi):
        C_pp_dd = C_res_vector.C_n_phiX_dd(m_d, m_phi, k_d, k_phi, T_d, xi_d, xi_phi, y2*y2*(c_th**8.)) / 4. # symmetry factor 1/4
        C_da = C_res_vector.C_n_3_12(m_d, m_a, m_phi, k_d, k_a, k_phi, T_d, T_a, T_d, xi_d,   0., xi_phi, M2_da)
        C_aa = C_res_vector.C_n_3_12(m_a, m_a, m_phi, k_a, k_a, k_phi, T_a, T_a, T_d,   0.,   0., xi_phi, M2_aa) / 2.
        print('C_ns:', C_pp_dd, C_da, C_aa)
        return C_da + 2.*C_aa + 2.*C_pp_dd
    def C_rho(T_a, T_d, xi_d, xi_phi):
        C_da = C_res_vector.C_rho_3_12(2, m_d, m_a, m_phi, k_d, k_a, k_phi, T_d, T_a, T_d, xi_d, 0., xi_phi, M2_da)
        C_aa = C_res_vector.C_rho_3_12(3, m_a, m_a, m_phi, k_d, k_a, k_phi, T_a, T_a, T_d,   0., 0., xi_phi, M2_aa) / 2. # symmetry factor 1/2
        return C_da + C_aa
    def C_xi0(T_a, T_d, xi_d, xi_phi):
        C_pp_dd = C_res_vector.C_n_phiX_dd(m_d, m_phi, k_d, k_phi, T_d, xi_d, xi_phi, y2*y2*(c_th**8.), type=1) / 4.
        return 2.*C_pp_dd
    ...
    plt.loglog(m_d/Ttrel.T_nu_grid[i_ic:i_end+1], pan.n_phi_grid_sol/ent_grid[i_ic:i_end+1], color='darkorange')
    plt.loglog(m_d/Ttrel.T_nu_grid[i_ic:i_end+1], (pan.n_chi_grid_sol+2.*pan.n_phi_grid_sol)/ent_grid[i_ic:i_end+1], color='mediumorchid')
    plt.show()

    plt.loglog(m_d/Ttrel.T_nu_grid[i_ic:i_end+1], pan.T_chi_grid_sol*np.abs(pan.xi_chi_grid_sol), color='dodgerblue')
    plt.loglog(m_d/Ttrel.T_nu_grid[i_ic:i_end+1], pan.T_chi_grid_sol*np.abs(0.5*pan.xi_phi_grid_sol), color='darkorange')
    plt.show()
    ...
    i_skip = 100
    C_dd = np.array([-C_res_vector.C_n(m_d, m_d, m_phi, k_d, k_d, k_phi, T_d, T_d, T_d, xi_d, xi_d, xi_phi, M2_dd, type=-1) / 2. for T_d, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_inv_dd = np.array([C_res_vector.C_n(m_d, m_d, m_phi, k_d, k_d, k_phi, T_d, T_d, T_d, xi_d, xi_d, xi_phi, M2_dd, type=1) / 2. for T_d, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_da = np.array([-C_res_vector.C_n(m_d, m_a, m_phi, k_d, k_a, k_phi, T_d, T_a, T_d, xi_d, 0., xi_phi, M2_da, type=-1) for T_d, T_a, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.T_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_inv_da = np.array([C_res_vector.C_n(m_d, m_a, m_phi, k_d, k_a, k_phi, T_d, T_a, T_d, xi_d, 0., xi_phi, M2_da, type=1) for T_d, T_a, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.T_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_aa = np.array([-C_res_vector.C_n(m_a, m_a, m_phi, k_d, k_a, k_phi, T_a, T_a, T_d, 0., 0., xi_phi, M2_da, type=-1) / 2. for T_d, T_a, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.T_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_inv_aa = np.array([C_res_vector.C_n(m_a, m_a, m_phi, k_d, k_a, k_phi, T_a, T_a, T_d, 0., 0., xi_phi, M2_da, type=1) / 2. for T_d, T_a, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.T_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_ann = np.array([-C_res_vector.C_n_phiX_dd(m_d, m_phi, k_d, k_phi, T_d, xi_d, xi_phi, y2*y2*(c_th**8.), type=-1) / 4. for T_d, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    C_inv_ann = np.array([C_res_vector.C_n_phiX_dd(m_d, m_phi, k_d, k_phi, T_d, xi_d, xi_phi, y2*y2*(c_th**8.), type=1) / 4. for T_d, xi_d, xi_phi in zip(pan.T_chi_grid_sol[::i_skip], pan.xi_chi_grid_sol[::i_skip], pan.xi_phi_grid_sol[::i_skip])])
    ...
    plt.loglog(m_d/Ttrel.T_nu_grid[i_ic:i_end+1:i_skip], 3.*Ttrel.hubble_grid[i_ic:i_end+1:i_skip]*pan.n_phi_grid_sol[::i_skip], color='mediumorchid')
    plt.show()
