...
Error-message:
something went wrong...  Unable to allocate 1.16 MiB for an array with shape (38014, 4) and data type float64 <class 'numpy.core._ephiceptions._ArrayMemoryError'> find_y.py 73
something went wrong...  Allocation failed (probably too large). <class 'MemoryError'> find_y.py 73
"""

r_m = 3.        # Anton: Mass ratio m_phi/m_d
m_a = 0.        # Anton: Take SM-neutrino mass to be zero
k_d = 1.        # Anton: Fermi-statistics for fermion
k_a = 1.        
k_phi = -1.       # Anton: Bose-statistics for boson
dof_d = 2.      # Anton: Fermion 2 spin dof.
dof_phi = 1.      # Anton: real scalar has 1 dof.
...
# The mass is given in GeV, but is plotted in kev. Use 1e-6 * phi GeV = phi kev
n_m = 5         # 21
n_th = 10        # 81
...
def find_y(params):
    ...
    mp_str = f'{m_phi:.5e}'.split('e')[0].rstrip('0').rstrip('.') + 'e' + f'{m_phi:.5e}'.split('e')[1]
    ...
    filename = f'md_{md_str};mphi_{mp_str};sin22th_{sin22th_str};y_{y_str};full.dat'
    ...
    phi_therm = m_d/T_nu_grid[np.argmax(therm_ratio > 1.)]
    phi_d_therm = m_d/T_d_grid[np.argmax(therm_ratio > 1.)]
    ...
    return m_d, m_phi, sin2_2th, y_cur, O_d_h2, phi_therm, phi_d_therm, therm_ratio_max, fs_length, fs_length_3, T_kd, T_kd_3, T_d_kd, T_d_kd_3, r_sound, r_sound_3
...
if __name__ == '__main__':
    ...
    print(dirname+f'rm_{r_m:.2e}_y_relic_test_{n_m}phi{n_th}phi{i_max}_q1.dat')
    np.savetxt(dirname+f'rm_{r_m:.2e}_y_relic_test_{n_m}phi{n_th}phi{i_max}_q1.dat', results)
