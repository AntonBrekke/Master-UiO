...
class TimeTempRelation(object):
    def __init__(self, T_start=1e8, t_end=t_max, t_gp_pd=100, m_psi=None, dof_psi=None, k_psi=None):
        if m_psi is None:
            self.psi_in_SM = True
        else:
...
class Pandemolator(object):
    def __init__(self, m_chi, k_chi, dof_chi, m_phi, k_phi, dof_phi, m_psi, k_psi, C_n, C_rho, C_xi0, t_grid, T_grid, dT_dt_grid, ent_grid, hubble_grid, sf_grid, i_ic, n_ic, rho_ic, i_end):
        ...
        self.C_xi0 = C_xi0 # part of rhs of Boltzmann-eq. for n setting xi = 0 (i.e. chi chi -> phi phi)
        ...
        self.rho_ic = rho_ic # initial condition for rho_chi + rho_phi
    ...
    def jac_n_rho_root(self, Txi_chi, n_in, rho_in):
        ...
        rho_der_xi = self.rho_chi_der_xi(T_chi, xi_chi) + self.fac_n_phi*self.rho_phi_der_xi(T_chi, self.fac_n_phi*xi_chi)
    ...
    def event_xi_zero(self, log_x, y):
        ...
        H = self.H_interp_T(T)
        dT_dt = self.dT_dt_interp_T(T)
        ent = self.ent_interp_T(T)          # Anton: Entropy - can be both negative and positive
        sf = self.sf_interp_T(T)
        Y = y[0]
        n = Y * ent         # Anton: Number density 
        rho = y[1] / (sf**4.)
    ...
    def pandemolate(self):
        ...
        dof_fac_phi = self.dof_phi if self.k_phi == -1 else self.dof_phi*7./8.
        self.T_chi_last = (self.rho_ic / (cf.pi2*(dof_fac_chi+dof_fac_phi)/30.))**0.25
        self.xi_chi_last = 0.
        ...
        while i_max < n_pts - 1:
            ...
            if i_xi_nonzero < n_pts - 1:
                ...
                event_abund.terminal = True
                event_abund.direction = -1
                print('Start solve_ivp self.der in pandemolate ')
                time1 = time.time()
                ...
                print(f'End solve_ivp self.der in pandemolate {time.time()-time1}s later ')
